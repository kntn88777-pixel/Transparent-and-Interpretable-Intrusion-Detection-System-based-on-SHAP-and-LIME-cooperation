...
DATASET_FILES = [
    {"name": "CIC17", "X_train": "preprocessed_cic17_X_train.npy", "X_test": "preprocessed_cic17_X_test.npy",
     "Y_train": "preprocessed_cic17_y_train.npy", "Y_test": "preprocessed_cic17_y_test.npy"},
    {"name": "NSL-KDD", "X_train": "preprocessed_nsl-kdd_X_train.npy", "X_test": "preprocessed_nsl-kdd_X_test.npy",
     "Y_train": "preprocessed_nsl-kdd_y_train.npy", "Y_test": "preprocessed_nsl-kdd_y_test.npy"},
    {"name": "SIGNAL2022", "X_train": "signal22_X_train.npy", "X_test": "signal22_X_test.npy",
     "Y_train": "signal22_y_train.npy", "Y_test": "signal22_y_test.npy"},
]
...
# ────────────────────────────────────────────────
# DEEP LEARNING MODELS (có early stopping)
# ────────────────────────────────────────────────
def train_dnn(X_train, y_train, X_test):
    ...
    history = model.fit(
        X_train, y_train,
        epochs=30,
        batch_size=256,  # tăng lên để nhanh hơn
...
def train_lstm(X_train, y_train, X_test):
    ...
    model = Sequential([
        LSTM(96, input_shape=(1, X_train.shape[1])),  # giảm từ 128 → nhẹ hơn
        Dropout(0.3),
        Dense(64, activation='relu'),
        Dense(num_classes, activation='softmax')
    ])
...
# ────────────────────────────────────────────────
# MODELS (giảm tải CPU/RAM)
# ────────────────────────────────────────────────
# Trong def get_models(num_classes):   # num_classes = len(np.unique(y_train))

def get_models(num_classes):
    is_multiclass = num_classes > 2   # >2 thì multiclass, ==2 là binary (LightGBM xử lý tốt), ==1 thì lỗi dữ liệu
    ...
    if is_multiclass:
        ...
        lgb_params['objective'] = 'binary'   # hoặc 'cross_entropy' nếu muốn
...
def main():
    ...
    print("\nHoàn tất! Kết quả đã lưu vào results/model_results.xlsx")
    print("Model đã lưu trong thư mục: saved_models/")
    print("Biểu đồ đã lưu trong thư mục: plots/")


if __name__ == "__main__":
    tf.get_logger().setLevel('ERROR')  # giảm log thừa của tensorflow
    main()